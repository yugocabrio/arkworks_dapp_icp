...
// （なんちゃって）Trusted Setupによって生成したProvingKey
// ProvingKey generated by Trusted Setup
...
pub fn create_proof(a: u32, b: u32) -> Result<Vec<u8>, JsValue> {
    ...
    // cはPublic inputですが、検証側で固定しています。
    // c is Public input, but is fixed on the verification side.
...
// フロントエンドでProofを表示するために整形しています。
// Formatted to display Proof on front end.
#[wasm_bindgen]
